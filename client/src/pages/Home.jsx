  ...
  useEffect(() => {
    axios
      .get("https://api.api-ninjas.com/v1/randomuser", {
        headers: {
          "X-Api-Key": "CPX1n5e+7IijyO6G30sQvg==IVX3zJCpIcL2Paro",
        },
      })
      .then((response) => {
        console.log(response.data, " random username and other details  ");
        setUsername_[response.data.username];
        window.sessionStorage.setItem("username_", response.data.username);
      })
      .catch((error) => {
        console.log(
          error,
          " error happened while fetching random username \n "
        );
      });
  }, []);
